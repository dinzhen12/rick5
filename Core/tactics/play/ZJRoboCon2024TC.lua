...
local checkEnemyRobotInCircle = function(num, circleNum)
    ...
    return checkInCircle(enemy.pos(num), circleNum, PLAYER_RADIUS)
end
...
local updateLastTouch = function()
    ...
    for i=0,param.maxPlayer-1 do
        if enemy.valid(i) then
            if (enemy.pos(i) - ball.pos()):mod() < PLAYER_RADIUS + BALL_RADIUS then
                team = false
                robotNum = i
                break
            end
    ...
    for i=0,param.maxPlayer-1 do
        if player.valid(i) then
            if (player.pos(i) - ball.pos()):mod() < PLAYER_RADIUS + BALL_RADIUS then
                team = true
                robotNum = i
                break
            end
